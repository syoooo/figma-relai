...
Relai 的出发点很简单：AI 时代，设计师的主导权应该变大，而不是变小。判断和品味留在你手里；活交给你已经信任的模型——每一步都看得见。
...
Relai 的作者自己就在用它维护一套生产中的设计系统。摘自其中一晚的会话——2026 年 7 月，数字未经修饰：
...
审计。`analyze_design` 检查颜色 token 覆盖率、auto-layout 质量、组件健康度和可访问性（WCAG 对比度、触摸目标、文字尺寸），也可以四项合一输出加权 0–100 健康分，直接贴进评审。另有 agent-readiness 就绪分（文件对 AI 工作的准备度与最大缺口）、文件声音指纹（圆角/间距/字号签名）、以及清点已删除变量残留引用的幽灵普查。
...
插件就是设计师这一侧的窗口：AI 一举一动尽收眼底的活动流；"AI 已连接"指示灯亮起意味着智能体真的配对上了，而不只是某个服务器在跑；停止按钮随时取消待执行的工作。你的选区和页面切换会作为事件流回 AI，"现在对这个做同样的事"不需要重新解释。中继在本地运行：文件内容只在 Figma、你的机器和你已经信任的 AI 客户端之间流动。

想握得更紧，还有三道开关。**审批**("大改动前先确认")会把批量写入和代码执行拦在面板里，等你按下批准。**仅限选区**阻止选区之外的一切改动——AI 收到的是明确报错，不是默许。**文件规约**则是存在 Figma 文件体内的一份小 CLAUDE.md：命名规则、间距习惯、禁碰页面——此后每个会话，无论来自哪个 AI 客户端，动手前都会先读它。从 0.3 起，文件还会有**记忆**：你的裁决（「这个偏离是意图，不是漂移」）会作为判例记录在文件里，此后任何触到其引用对象的编辑，结果中都会附上该判例——AI 在动手的瞬间被你自己的判例法纠正。面板的 MEMORY 行可查看全部条目，随时可删。**AI 禁区**按页设防：写入禁区页会在调度层被拒绝，禁区名单只有你能改。确认统一为四档刻度盘（OPEN · RISK · BULK · ALL）：默认 RISK 档只拦高危操作（删变量/样式、detach、flatten）——branch 工作流拨到 OPEN 即可。界面支持 English、日本語、中文。
...
每个工具都是自描述的，AI 能看到完整参数文档。这份契约也以文件形式存在：`npx figma-relai manifest` 输出机器可读的 JSON——全部工具 schema、插件命令表、已知陷阱，每次构建从运行中的代码生成(以 `docs/manifest.json` 入库)，结构上不可能与实现漂移；`npx figma-relai docs <tool>` 则把同一份渲染给人看。随包附带 11 份 skill 文档，以 MCP prompts 形式提供：token 策略、组件规约、审计工作流、QA 体检、`execute_figma` 的 Plugin API 速查表、文件记忆与判例，以及设计系统优先建屏、批量清理、评论驱动协作的实战配方。你自己的 skill 也能装载：把带 name/description frontmatter 的 markdown 放进 `~/.figma-relai/skills/`，即注册为 `user:` 前缀的 prompt。
