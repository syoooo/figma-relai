...
检查。`analyze_design` 检查颜色 Token 覆盖、自动布局质量、组件健康度和无障碍性（WCAG 对比度、触达目标、字号），也能合成一个加权 0–100 的健康分直接放进评审。另有 agent 就绪度评分（文件为 AI 工作准备到什么程度，附最大缺口）、文件声纹（圆角/间距/字体的签名）、以及清点指向已删除变量的幽灵普查。
