...
想握得更紧，还有三道开关。**审批**("大改动前先确认")会把批量写入和代码执行拦在面板里，等你按下批准。**仅限选区**阻止选区之外的一切改动——AI 收到的是明确报错，不是默许。**文件规约**则是存在 Figma 文件体内的一份小 CLAUDE.md：命名规则、间距习惯、禁碰页面——此后每个会话，无论来自哪个 AI 客户端，动手前都会先读它。从 0.3 起，文件还会有**记忆**：你的裁决（「这个偏离是意图，不是漂移」）会作为判例记录在文件里，此后任何触到其引用对象的编辑，结果中都会附上该判例——AI 在动手的瞬间被你自己的判例法纠正。面板的 MEMORY 行可查看全部条目，随时可删。界面支持 English、日本語、中文。
