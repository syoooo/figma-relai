...
export const translations = {
  ...
  ja: {
    ...
    craft: {
      ...
      items: [
        { kicker: 'pitfalls', tag: 'RAW ERROR + HINT', title:'落とし穴には、地図がある。', body: 'Plugin APIは落とし穴だらけです。Relaiには実際の運用で踏んだ41件が同梱されています。踏めば、生のエラーに解決策が添えられて返ってきます。同じ一覧は、AIがコードを書く前に読むチートシートにもなります。', artifact: '✗ cannot write to node with unloaded font\n  hint: await figma.loadFontAsync(node.fontName)\n        before editing text — new TextNodes\n        default to Inter Regular' },
  ...
  zh: {
    ...
    craft: {
      ...
      items: [
        { kicker: 'pitfalls', tag: 'RAW ERROR + HINT', title:'雷区，已绘制成图。', body: 'Plugin API处处是坑。Relai内置了41条生产环境踩出来的 —— 一旦踩中,原始报错连同解法一起返回;同一份清单,就是AI写代码前要读的速查表。', artifact: '✗ cannot write to node with unloaded font\n  hint: await figma.loadFontAsync(node.fontName)\n        before editing text —— new TextNodes\n        default to Inter Regular' },
