...
export const translations = {
  en: {
    ...
    craft: {
      ...
      body: 'Connecting an AI to Figma is the easy part. Most of Relai is the unglamorous work that decides whether the output is production-grade or plausible-looking noise — and eighty-seven tests written to break it gate every release that touches logic.',
      items: [
        { kicker: 'pitfalls', tag: 'RAW ERROR + HINT', title:'The minefield, mapped', body: 'The Plugin API is full of traps. Relai ships 25 of them, learned in production — trip one, and the raw error comes back with the fix attached; the same registry becomes the cheat sheet the AI reads before writing code.', artifact: '✗ cannot write to node with unloaded font\n  hint: await figma.loadFontAsync(node.fontName)\n        before editing text — new TextNodes\n        default to Inter Regular' },
        ...
        { kicker: 'navigation', tag: 'TOOL RESULT', title:'A map, not a maze', body: 'Thirty-two tools, each answering with a summary, honest notes on what got truncated, and a recommended next step. The AI arrives briefed, too — eleven skills as MCP prompts, your own from `~/.figma-relai/skills`, the whole contract as a `manifest` regenerated from running code, and one `doctor` command that diagnoses the chain end to end.', artifact: '{\n  "summary": "3 pages · 45 component sets",\n  "note": "components truncated (top 40)",\n  "recommended_next": "get_node_details\n    on the set you plan to extend"\n}' }
      ]
    },
    outlook: {
      eyebrow: 'NOW & NEXT',
  ...
  ja: {
    ...
    craft: {
      ...
      body: 'AIをFigmaにつなぐこと自体は簡単です。Relaiの中身の大半は、出力を「それっぽい何か」で終わらせないための、目立たない作り込みです。ロジックに触れるリリースは、87項目の意地悪な検査が門番をしています。',
      items: [
        { kicker: 'pitfalls', tag: 'RAW ERROR + HINT', title:'落とし穴には、地図がある。', body: 'Plugin APIは落とし穴だらけです。Relaiには実際の運用で踏んだ25件が同梱されています。踏めば、生のエラーに解決策が添えられて返ってきます。同じ一覧は、AIがコードを書く前に読むチートシートにもなります。', artifact: '✗ cannot write to node with unloaded font\n  hint: await figma.loadFontAsync(node.fontName)\n        before editing text — new TextNodes\n        default to Inter Regular' },
        ...
        { kicker: 'navigation', tag: 'TOOL RESULT', title:'結果には、次の一手がついてくる。', body: '32個のツールは、どれも要約・省略箇所の正直な注記・推奨される次の一手を添えて答えます。しかもAIは、下調べ済みで現場に来ます。11本のスキルがMCPプロンプトとして同梱され、自作スキルは`~/.figma-relai/skills`から読み込まれ、契約の全体は実行コードから再生成される`manifest`になっています。調子が悪ければ、`doctor`一発で診断です。', artifact: '{\n  "summary": "3 pages · 45 component sets",\n  "note": "components truncated (top 40)",\n  "recommended_next": "get_node_details\n    on the set you plan to extend"\n}' }
      ]
    },
    outlook: {
      eyebrow: 'NOW & NEXT',
  ...
  zh: {
    ...
    craft: {
      eyebrow: 'CRAFT',
      title: '结果更好，是有原因的。',
      body: '把AI接到Figma上并不难。Relai的大部分工作，是那些决定输出到底是生产级、还是"看着像回事的噪音"的朴素功夫 —— 87道故意找茬的检查,守在每次触及逻辑的发布之前。',
      items: [
        { kicker: 'pitfalls', tag: 'RAW ERROR + HINT', title:'雷区，已绘制成图。', body: 'Plugin API处处是坑。Relai内置了25条生产环境踩出来的 —— 一旦踩中,原始报错连同解法一起返回;同一份清单,就是AI写代码前要读的速查表。', artifact: '✗ cannot write to node with unloaded font\n  hint: await figma.loadFontAsync(node.fontName)\n        before editing text —— new TextNodes\n        default to Inter Regular' },
        ...
        { kicker: 'navigation', tag: 'TOOL RESULT', title:'给的是路标，不是迷宫。', body: '32个工具,每个都带着摘要、对截断之处的诚实标注、建议的下一步来回答。而且AI是备过课进场的 —— 11份skill作为MCP prompts内置,你自己写的从`~/.figma-relai/skills`装载,整套契约是随构建从运行代码重新生成的`manifest`;不对劲时,一条`doctor`命令端到端诊断。', artifact: '{\n  "summary": "3 pages · 45 component sets",\n  "note": "components truncated (top 40)",\n  "recommended_next": "get_node_details\n    on the set you plan to extend"\n}' }
      ]
    },
    outlook: {
      eyebrow: 'NOW & NEXT',
