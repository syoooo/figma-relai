...
各ツールは自己記述的で、AI にはパラメータの完全なドキュメントが見えます。トークン戦略、コンポーネント規約、監査ワークフロー、`execute_figma` 用 Plugin API チートシートに加え、デザインシステム起点の画面構築・一括クリーンアップ・コメント駆動コラボのレシピ — 計 9 つのスキルドキュメントが MCP prompts として同梱されます。
