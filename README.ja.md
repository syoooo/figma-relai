...
さらに踏み込みたいときのダイヤルが 3 つ。**承認**(「大きな編集の前に確認」)は一括書き込みやコード実行を、パネルで承認を押すまで待たせます。**選択範囲にロック**は選択の外への編集を拒否 — AI には黙認ではなく明確なエラーが返ります。そして**ファイル規約**は Figma ファイル自体に保存される小さな CLAUDE.md。命名規則、余白の流儀、触ってはいけないページ — 以後のセッションはどの AI クライアントからでも、作業前にこれを読みます。UI は English・日本語・中文に対応。
...
Figma 自身の AI は急速に進化しました — 公式 MCP サーバーはキャンバスへの書き込みに対応し、Figma Design Agent はエディタの中で一緒に作業します。どちらも素晴らしく、そしてどちらも有料プランのフルシートのもの。使用量は AI クレジットで計られ、モデルは Figma が選びます。Relai はその線の外側にいる全員のためのオープンソースの対になる存在です:無料を含むすべてのプラン、いま契約しているモデルとサブスクリプション、すべてがあなたのマシン上で動き、主導権はデザイナーの手に。シートがあるなら、2 つは気持ちよく共存します — 両方どうぞ。
...
まず `npx figma-relai doctor` を。Node、リレーのポート(無関係なプロセスが占有していないかも)、プラグインの在席、保存済みルーム、コメント用トークンを一発で点検し、それぞれの直し方を添えて返します。
