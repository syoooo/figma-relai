...
- **The no-go zones stopped accusing you of reading.** A script that only looked at the file — listing page backgrounds, reading the guards value — was reported as having written to a guarded page, because the check took every `1234:5678` in the returned JSON as something the script had touched. Two separate ways of being wrong: a returned *page* id resolved to "the page itself", so merely naming a no-go zone in a result looked like editing it; and the pattern also sliced the main component's id out of the middle of an instance sublayer's, so fixing an override on one page was reported against the component's page somewhere else. Ids are read whole now, and only scene nodes count — a page id in a result is data, not a confession. Six false alarms in one afternoon is how a real warning gets trained into wallpaper.
- **Restore will not walk backwards over newer law.** The file's conventions carried no timestamp, so "these differ from the kit" was the most the panel could say — and the only button it had ran kit → file. On one afternoon that replaced a law written minutes earlier with a month-old ancestor, silently, because nothing in the product was comparing the two dates. The file's law is stamped when written; drift now names the side that was written last, the card leads with **Send mine to the kit** when that side is the file, and a restore that would destroy newer writing is refused with both sizes and both dates until you say `force`. Healing a merge wound is unchanged — an empty file has nothing to lose.
- **A kit that refuses a write says so.** The ruleset store reported a save, then came back at its previous size after the plugin restarted; a kit that looks like a backup while quietly holding a stale copy is worse than no kit at all, because it is trusted and then restored from. Every write is read back and compared, and a store that did not keep it now fails loudly instead of leaving you to find out at the next merge.
- **The plugin can move rooms without going missing.** Reopening it, or switching a file to one of its branches, hands it a fresh room name — and pairing was chosen once and never revisited, so every command answered "the Figma plugin is not open" about a plugin sitting right there, while `join_room` cheerfully listed the live one. A room that goes quiet is now checked against the relay: exactly one plugin elsewhere and the session follows it, several and they are named so you can choose.

## 0.7.6

- **The stop control stopped shoving the header around, and stopped appearing for work nobody wants to stop.** It was a bordered box that flicked in and out with `display`, inside a group pinned to the right edge — so every command start pushed the history link 62.6 pixels sideways and every settle pushed it back, for a 0.1-second read as readily as for anything worth interrupting. It is a word now, set into the leader rule between the section name and the elastic dashes, which pay for its width out of their own: across idle, armed, armed-with-a-count and hovered, the history link does not move by a thousandth of a pixel and the row keeps the same height as the rule below it. It opens and closes on the transition the activity rows already use, because things in this panel open and close rather than appear. And it is bound to a busy *streak* rather than to "is something running": twenty serial short commands are one streak that opens the slot once, where the old button blinked twenty times. Nothing under two seconds raises it at all.
- **It says how much it will take.** `stopAll` has always been global, and a single word for a global abort is concealment by omission — when more than the one visibly pulsing row is in flight, the control reads `STOP ×3`. Hovering it lights the dashed rule from the control to the right edge: the span that lights is the section, and the section is what will be stopped. The history link keeps its own colour, because it is not implicated.
- **The panel's only bordered control is gone with it,** and so is the filled square that prefixed it — a filled square in this panel is a status lamp, not a verb.
...
- **Relai finds the wiring that isn't there.** A component set can declare a property no variant references: the panel shows the control, the designer toggles it, nothing happens, and nothing is ever raised. `execute_figma` reports it now, and `validate_design_rules` gained a `property_wiring` rule for the same fault. A second rule catches the one that produced it more than once — `clone()` downgrades a SLOT child to a plain FRAME and drops its `slotContentId`, so a copied variant loses its slot while looking identical to its siblings. Documentation had already failed at this twice: neither fault throws, so only a declared-versus-referenced comparison finds them. Reaching one of the four dead properties in a real design system took a single scan.
- **The plugin can finally say which file it is looking at.** `figma.fileKey` is gated to private organisation plugins, so a Community plugin always read `null` and "auto-detected from the open plugin when possible" was never once possible. It is resolved the long way now — a published component's key names the main file, and the branch list names the branch. That last hop is not bookkeeping: comments live per branch, so three pins on a branch used to come back as an empty list from the parent file. The answer is cached in the plugin's own storage, never in the designer's file.
- **A comment is somewhere, and now it says where.** `client_meta` arrives in four documented shapes plus at least one undocumented field, and everything but `node_id` was being thrown away — a pin on a component and a pin on empty canvas read identically, and a region comment covering an area did not report that it was a region. It is passed through whole, and each pin is resolved to a node, a page and a canvas position, because an offset inside an unknown frame locates nothing.
- **The access-token lamp reports whether the token works, not whether one exists.** An expired token lit green while every REST call failed, and library discovery told you "every candidate 404'd" about a run of 403s. Validity is checked once at startup and again the moment Figma refuses a call.
- **The scope lock covers a page, and survives a reload.** A PAGE cannot be selected in Figma, so there was no way to say "only this page" — flipping the switch with nothing selected now means exactly that. And the lock persists: coming back off after a plugin reload is a safety regression, not a convenience.
- **"The Figma plugin is not open" stops being said about a plugin that is open.** Presence was cached and only ever refreshed by a pushed event, so a plugin that closed and reopened could leave the cache stuck on false — while `join_room`, which asks the relay instead of trusting the cache, listed it happily. It asks now.
- **The panel says which branch, how far along, and what is moving.** The file card carries the parent file with the branch trailing behind a git-branch mark that never gets truncated; progress stops being a bare number; and the activity feed opens and closes for a row instead of jolting, with a mark that breathes while a command runs. Three more pitfalls, 57 now, including the one where a retired variable keeps resolving and keeps rendering because a consumer scan stopped at the first instance.
...
- **The warning arrives with the token, not with the audit.** `manage_variables action:create` without scopes answers with the scope the token's own name implies — `Card/border-radius` reads as `CORNER_RADIUS`, `Link/text-color/hovered` as `TEXT_FILL`. A token born at ALL_SCOPES is offered for every property in the file, and it costs nothing to narrow in the same breath and a hundred edits to narrow later.
- **`validate_design_rules` gained the two faults that look perfect on the canvas**: `token_scopes` (a bound token still at ALL_SCOPES) and `token_borrowing` (a token that aliases another component's token, inheriting changes nobody meant to send it). Both are node-scoped, so a component can be checked the moment it is finished. They share one alias graph with the file-wide report — a criterion kept in two places becomes two criteria.
